...
local subm = stater(states2, stater.FLAG_CLEANUP_EVERY)
local csm  = stater(states3, stater.FLAG_CLEANUP_EVERY)
...
local sm = stater(states, (stater.FLAG_CLEANUP_DONE|stater.FLAG_CLEANUP_EVERY))
...
if arg.val ~= 9123 then
    print(string.format("\tFail: val = %d, expected %d", arg.val, 9123))
    fail = true
end

if arg.val2 ~= -82 then
    print(string.format("\tFail: val = %d, expected %d", arg.val2, -82))
    fail = true
end
